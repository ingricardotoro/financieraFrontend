...
function PagarView(props) {
    ...
    console.log("HIS="+JSON.stringify(history))
    ...
    const toggle = () => {setModal(!modal); setViewPagar(false)}
    ...
    const GetPayment = async()=>{
        ...
        CalcularMora(resp_payment.data.payment.dateToPay, resp_payment.data.payment.amountToPay)
    ...
    const updateRequerido = (action, value)=>{
        ...
        let valor = 0
        if(action ==="other"){valor = parseFloat(totalDeuda) + parseFloat(value) - parseFloat(descuento)}
        if(action ==="desc"){valor = parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(value)}
    ...
    const Pagar = async ()=>{
        ...
        if(parseFloat(amountPayed) !== 0  ){
            ...
            //if(parseFloat(amountPayed).toFixed(2) >=  ((parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento)).toFixed(2))){
                ...
                try {
                    console.log("1")
                    await Axios.put(URL_API+'/payments/updateState/'+paymentID, dataPayment)
                    setModal(!modal); 
                    setViewPagar(false)
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-12 col-md-6" style={{textAlign:"center"}}>
                                        ...
                                        <div style={{textAlign:"center"}}>
                                            <h3 htmlFor="">LPS. { (parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento) - parseFloat(payment.amountPayed)).toFixed(2)}</h3>
                                        </div>
                                        ...
                                        <div className="d-flex m-auto" style={{width:"90%"}}>
                                            ...
                                            <div className="input-group input-group-lg" >
                                                <input value={amountPayed} name="amountPayed" onChange={handleAmountPayed} type="number" min="0" className="form-control"  placeholder="Cantidad a Pagar" style={{border:"3px solid green"}} />
                                            </div>
                                        </div>
                                            <label >Tipo de Pago</label>
                                        <div className="d-flex m-auto" style={{width:"80%"}}>

                                            <select value={methodPayed} onChange={handleInputSelect}  name="methodPayed" id="methodPayed" className="form-control m-t-5"> 
                                    ...
                                    <div className="col-sm-12 col-md-6 " >
                                           ...
                                           <div className="input-group input-group-lg" style={{width:"90%"}}>
                                                <button onClick={()=>Pagar()} style={{width:"100%"}} className="btn btn-lg btn-success">PAGAR</button>
                                            </div>
                                    
                                           
                                            <label >Otro Pago</label>
                                            <div className="input-group input-group-lg" style={{width:"90%"}}>
                                                <input value={otherPay} name="otherpay" onChange={handleOtherPay} type="number" min="0" className="form-control"  placeholder="Registrar Otro pago" />
                                            </div>

                                            <label >Descuento</label>
                                            <div className="input-group input-group-lg" style={{width:"90%"}}>
                                                 <input value={descuento} name="discount" onChange={handleDescuento} type="number" min="0" className="form-control"  placeholder="Valor de Descuento" />
                                            </div>
                                    </div>
                                    ...
                                    <div className="table-responsive ">
                                        <table className="table ">  
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td scope="col" style={{color:'red'}}>Pago Atrasado <p>del {NombreDay(new Date(payment?.dateToPay))}</p><p> {FormatDate(payment?.dateToPay)} </p></td>

                                                    <td scope="col">LPS. {parseFloat(payment?.amountToPay).toFixed(2)} </td>
                                                 ...
                                                 {/*    <td scope="col" > <p>{ DiasDeMora(payment?.dateToPay)} </p> <p> { " de retraso"}</p>  </td>
                                                    ...
                                                    <td scope="col" > <p>{ diasDemora} </p> <p> { " de retraso"}</p>  </td>
                                                    ...
                                                    <td scope="col">LPS. {parseFloat(payment?.amountPayed).toFixed(2)} </td>

                                                    {payment?.amountPayed > 0 ? 
                                                        <td scope="col">LPS. {(parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento) - parseFloat(payment?.amountPayed).toFixed(2)).toFixed(2)}</td>
                                                    :
                                                        <td scope="col">LPS. {(parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento)).toFixed(2)}</td>
                                                    }
                                                ...
                                                <tr>
                                                    <td scope="col" style={{color:'green'}}>Próximo Pago {NombreDay(new Date(payment?.dateToPay))} <p>{FormatDate(payment?.dateToPay)} </p></td>
                                                    ...
                                                    {payment?.amountPayed > 0 ? 
                                                        <td scope="col">LPS. {(parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento) - parseFloat(payment?.amountPayed)).toFixed(2)}</td>
                                                    :
                                                        <td scope="col">LPS. {(parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento)).toFixed(2)}</td>
                                                    }
                                                  ...
                                                  {   <tr>
                                                        ...
                                                        <td ></td>
                                                        <td ></td>
                                                        <td ><strong>LPS. {(parseFloat(totalDeuda) + parseFloat(otherPay) - parseFloat(descuento) - parseFloat(payment?.amountPayed)).toFixed(2)}</strong></td>
