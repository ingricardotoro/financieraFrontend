...
export const ViewLoan = (props) => {
  ...
  if (Request?.frequency === 'Quincenal') {
    frecuency = 14;
  }
  ...
  const obtenerPagos = async () => {
    const resp_payments = await Axios.post(
      URL_API + '/payments/findPaymentsByLoanId/' + idLoan
    );
    ...
    for (let i = 0; i < pagos.length; i++) {
      ...
      if (pagos[i].statusPay === 'Pendiente') {
        if (today.diff(fechaRow) > 0) {
          // determinamos si la fecha de pago ya paso
          ...
          if (fechaRow2.diff(fechaRow) > 0) {
            //Pagos atrasados
            console.log('2');
            /*setValorI(i)
                        setIdNextPay(pagos[i]._id)*/
          } else {
            //Proximo pago a realizar
            console.log('3=' + i);
            setValorI(i);
            setIdNextPay(pagos[i]._id);
            setNextBool(true);
  ...
  const pagar = async () => {
    ...
    const resp_pay = await Axios.put(
      URL_API + '/payments/updateState/' + idNextPay,
      data
    );
  ...
  const CalcularMora = (dayToPay, cuota) => {
    ...
    if (duration > parseInt(frecuency)) {
      mora = (cuota * 0.03).toFixed(2); //para trabajar solo con 2 decimales aunque sea string
      moradiaria = (parseFloat(mora) / parseInt(frecuency)).toFixed(2); //para trabajar solo con 2 decimales aunque sea string
    }
    ...
    let totalToReturnString = (parseFloat(moradiaria) * duration).toFixed(2);
    let totalToReturn = parseFloat(totalToReturnString);
  ...
  //Control de pagos fila por fila
        ...
        <div className="main-body">
          <div className="page-wrapper">
            <div className="page-header mt-5">
              ...
              <div className="page-header-breadcrumb">
                <ul className="breadcrumb-title">
                  ...
                  <Link to="/prestamos">
                    <li className="breadcrumb-item">Volver a Préstamos</li>
                  </Link>
                </ul>
              </div>
            </div>
            <div className="page-body">
              ...
              <div className="card">
                <div className="card-header vertical">
                    ...
                    <div className="card card-contact borderless-card">
                      <div className="widget-profile-card-1">
                        ...
                        <img
                          ...
                          alt="card-style-1"
                        />
                        <button className="btn btn-default btn-outline-default btn-icon b-lft">
                          <i className="icofont icofont-ui-user" />
                        </button>
                        <button className="btn btn-default btn-outline-default btn-icon b-rgt">
                      ...
                      <div className="card-block text-center">
                        <h5>
                          {Request?.customerId?.personId?.name}{' '}
                          {Request?.customerId?.personId?.lastname}{' '}
                        </h5>
                        <p className="text-muted">
                          {Request?.customerId?.personId?.city}
                        </p>
                        <p className="text-muted">
                          {Request?.customerId?.personId?.identidad}
                        </p>
                        <p className="text-muted">
                          {Request?.customerId?.personId?.phone1} -{' '}
                          {Request?.customerId?.personId?.phone2}
                        </p>
                        <p className="text-muted">
                          {Request?.customerId?.personId?.email1}
                        </p>
                        ...
                        <div className="row text-center">
                          <div className="col-6 b-r-default">
                            <h4 className="text-primary">
                              {Request?.customerId?.codeCustomer}
                            </h4>
                  ...
                  <div className="col-sm-12 col-md-4">
                    <div className="card card-contact borderless-card">
                      <button
                        className="btn btn-xlg btn-success width-100 "
                        data-toggle="modal"
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td>
                                  <button
                                    onClick={() => {
                                      verAval(Request?.aval1Id);
                                    }}
                                    className="btn btn-warning btn-xs mr-3"
                                  >
                                    Aval 1
                                  </button>
                                  <button
                                    onClick={() => {
                                      verAval(Request?.aval2Id);
                                    }}
                        ...
                        <tr key={payment._id}>
                            ...
                            new Date(payment.dateToPay).getTime() < today ? ( //Pago con retraso
                              <Fragment>
                                <td>
                                  ...
                                  <p>
                                    {' '}
                                    {FormatDate(new Date(payment.dateToPay))}
                                  </p>
                                </td>
                                <td>
                                  <strong className="text-danger">
                                    {
                                      (CalcularPagoAtrasado(
                                        payment.amountToPay
                                      ),
                                      formatter.format(
                                ...
                                <td>
                                  <strong className="text-danger">
                                    {formatter.format(
                                      payment.amountToCapital.toFixed(2)
                                    )}
                                  </strong>
                                </td>
                                <td>
                                  <strong className="text-danger">
                                    {formatter.format(
                                      payment.amountToInteres.toFixed(2)
                                    )}
                                  </strong>
                                </td>
                                <td>
                              ...
                              <Fragment>
                                ...
                                <td>
                                  <strong className="text-warning">
                                    {formatter.format(
                                      payment.amountToMora.toFixed(2)
                                    )}
                                  </strong>
                                </td>
                                <td>
                          ...
                          {payment.statusPay === 'Pagada' ? (
                            <Fragment>
                              <td>
                                {NombreDay(new Date(payment.dateToPay))}{' '}
                                <p>{FormatDate(new Date(payment.dateToPay))}</p>
                              </td>
                              <td>
                              ...
                              <td>
                                ...
                                {moment(payment.datePayed).format('ll') >
                                moment(payment.dateToPay).format('ll') ? (
                                  <p className="text-danger">
                                    <strong>
                                      <p>{DiasDeMora(payment.dateToPay)}</p>{' '}
                                    </strong>
                                  </p>
                                ) : null}
                              </td>
                              ...
                              <td>
                                <strong className="text-warning">
                                  <button
                                    onClick={() => handleViewPago(payment._id)}
                          ...
                          {payment.statusPay === 'Abonado' ? (
                            <Fragment>
                              ...
                              <td>
                                <strong className="text-warning">
                                  <button
                                    onClick={() => handleViewPagar(payment._id)}
                                    className="btn btn-success"
                                  >
                                    Pagar
                                  </button>
              ...
              {viewPago === true ? (
                <PagoView paymentID={paymentID} setViewPago={setViewPago} />
              ) : null}
            </div>
                    ...
                    {/*  <div className="row">
                      ...
                      >
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <h3 htmlFor="">
                            LPS.{' '}
                            {parseFloat(payments[0]?.amountToPay).toFixed(2)}
                          </h3>
                        </div>
                        <hr />
                            ...
                            {nextBool === true ? ( //verificamos si hay pago proximo a pagar
                              <tr>
                                <td scope="col" style={{ color: 'green' }}>
                                  Próximo{' '}
                                  {NombreDay(
                                    new Date(payments[valorI]?.dateToPay)
                                  )}{' '}
                                  <p>
                                    {FormatDate(payments[valorI]?.dateToPay)}{' '}
                                  </p>
                                </td>

                                <td scope="col">
                                  LPS.{' '}
                                  {parseFloat(payments[0]?.amountToPay).toFixed(
                                    2
                                  )}{' '}
                                </td>
                            ...
                            {Arraymora.map((row, index) => (
                              <tr>
                                <td scope="col" style={{ color: 'red' }}>
                                  Pago Atrasado <p>{FormatDate(row.fechaI)} </p>
                                </td>

                                <td scope="col">
                                  LPS. {parseFloat(row.cuota).toFixed(2)}
                                </td>

                                <td scope="col" style={{ color: 'red' }}>
                                  {' '}
                                  {row.duration + ' dias de atraso'}{' '}
                                </td>

                                <td scope="col">
                                  LPS. {parseFloat(row.moraDeCuota).toFixed(2)}{' '}
                                </td>
                                ...
                                <td scope="col">
                                  LPS.{' '}
                                  {(
                                    parseFloat(row.cuota) +
                                    parseFloat(row.moraDeCuota)
                                  ).toFixed(2)}
                                </td>
                              </tr>
                            ))}
                            <tr>
                              ...
                              <td scope="col">
                                ...
                                {(
                                  ...
                                  parseFloat(payments[0]?.amountToPay)
                                ).toFixed(2)}
                              </td>
                            </tr>
                          </tbody>
